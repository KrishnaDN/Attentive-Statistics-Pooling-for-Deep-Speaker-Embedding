...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun  4 01:12:48 2020
...
import argparse
from models.Xvector_SAP import X_vector_Attn_Pooling
...
parser.add_argument('-evaluation_filepath',type=str, default='meta/evaluation.txt')

parser.add_argument('-input_dim', action="store_true", default=257)
parser.add_argument('-num_classes', action="store_true", default=462)
...
dataset_val = SpeechDataGenerator(manifest=args.evaluation_filepath,mode='train')
dataloader_val = DataLoader(dataset_train, batch_size=args.batch_size,shuffle=True,collate_fn=speech_collate) 

## Model related
use_cuda = torch.cuda.is_available()
device = torch.device("cuda" if use_cuda else "cpu")
model = X_vector_Attn_Pooling(args.input_dim, args.num_classes).to(device)
